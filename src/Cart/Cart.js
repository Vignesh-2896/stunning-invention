...
const Cart = (props) => {
    ...
    let updateQuantity = (arithmeticSymbol, ID) => {
        ...
        tempData.forEach(function(item,index,object){
            if(item["id"] === ID){
                if(arithmeticSymbol === "+") item["quantity"] = parseInt(item["quantity"]) + 1;
                else {
    ...
    return(
        <div className = "cart">
            <h3 onClick = {CloseCart} className = "closeBtn">&times;</h3>
            <h3 style = {{textAlign:"center"}}> Cart </h3>
            {cartData.map(function(item,index){
                return <CartItem key = {item["id"]} cartFunction = {updateQuantity} cartData = {item} />
            })}
            {cartData.length > 0
            ? (
            <div> 
...
let ShowCart = (pageName) => {    // Function to display the filter on click of the Filter Icon.
    ...
    document.getElementsByClassName("cart")[0].style.borderLeft = "2px solid black";
    document.getElementById("cartBtn").style.display = "none";
}

let CloseCart = () => {   // Function to hide the filter on click of the Close Icon.
